...
type C2RustUnnamed_7 = crate::core_transpiled::util::StackElement<*mut StackEntry>;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn _array__reserve(
    ...
    if new_capacity > (*self_0).capacity {
        if !((*self_0).contents).is_null() {
            (*self_0).contents = crate::core_transpiled::alloc::ts_realloc(
                (*self_0).contents,
                (new_capacity as libc::c_ulong).wrapping_mul(element_size),
            );
        } else {
            (*self_0).contents = crate::core_transpiled::alloc::ts_malloc(
                (new_capacity as libc::c_ulong).wrapping_mul(element_size),
            );
        }
...
unsafe extern "C" fn clock_is_null(mut self_0: TSClock) -> bool {
    return self_0.tv_sec == 0 && self_0.tv_nsec == 0;
}
#[inline]
...
static mut OP_COUNT_PER_PARSER_TIMEOUT_CHECK: libc::c_uint = 100 as libc::c_int as libc::c_uint;
...
unsafe extern "C" fn ts_parser__external_scanner_serialize(
    ...
    if ts_language_is_wasm((*self_0).language) {
        ...
        if length <= 1024 as libc::c_int as libc::c_uint {
        } else {
            panic!();
        }
        return length;
...
unsafe extern "C" fn ts_parser__lex(
    ...
    if ((*self_0).lexer.logger.log).is_some() || !((*self_0).dot_graph_file).is_null() {
        ...
        let mut off: libc::c_int = snwrite!(
            buf,
            1024 as libc::c_int as libc::c_ulong as usize,
            "lexed_lookahead sym:",
        )
...
unsafe extern "C" fn ts_parser__advance(
    ...
    loop {
        ...
        if (*self_0).operation_count == OP_COUNT_PER_PARSER_TIMEOUT_CHECK {
...
pub unsafe extern "C" fn ts_parser_new() -> *mut TSParser {
    let mut self_0: *mut TSParser = crate::core_transpiled::alloc::ts_calloc(
...
pub unsafe extern "C" fn ts_parser_parse(
    ...
    if ts_parser_has_outstanding_parse(self_0) {
        ...
        if (*self_0).has_scanner_error {
            current_block = 5735180597207344744;
        } else {
            if !old_tree.is_null() {
...
pub unsafe extern "C" fn ts_parser_set_wasm_store(
    ...
    if !((*self_0).language).is_null()
        && ts_language_is_wasm((*self_0).language) as libc::c_int != 0
    {
        let mut copy: *const TSLanguage = ts_language_copy((*self_0).language);
        ts_parser_set_language(self_0, copy);
