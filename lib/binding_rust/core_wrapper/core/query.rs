...
unsafe extern "C" fn ts_language_aliases_for_symbol(
    ...
    *end = (*start).offset(1 as libc::c_int as isize);
    ...
    loop {
        ...
        let mut symbol: TSSymbol = *((*self_0).alias_map).offset(fresh0 as isize);
        ...
        let fresh1 = idx;
        idx = idx.wrapping_add(1);
        let mut count: uint16_t = *((*self_0).alias_map).offset(fresh1 as isize);
...
unsafe extern "C" fn ts_lookahead_iterator__next(mut self_0: *mut LookaheadIterator) -> bool {
    if (*self_0).is_small_state {
        ...
        if (*self_0).data == (*self_0).group_end {
            ...
            let fresh2 = (*self_0).data;
            (*self_0).data = ((*self_0).data).offset(1);
            (*self_0).table_value = *fresh2;
            let fresh3 = (*self_0).data;
            (*self_0).data = ((*self_0).data).offset(1);
            let mut symbol_count: libc::c_uint = *fresh3 as libc::c_uint;
    ...
    *((*self_0).negated_fields.contents).offset(fresh34 as isize) = 0 as libc::c_int as TSFieldId;
    ...
    while stream.input < stream.end {
        ...
        *((*self_0).steps.contents).offset(fresh36 as isize) = query_step__new(
        ...
        (*pattern).end_byte = stream_offset(&mut stream);
...
pub unsafe extern "C" fn ts_query_end_byte_for_pattern(
    ...
    return (*((*self_0).patterns.contents).offset(pattern_index as isize)).end_byte;
}
#[no_mangle]
    ...
    loop {
        ...
        if (*self_0).ascending {
            ...
            if ts_query_cursor__should_descend(self_0, node_intersects_range) {
                ...
                {
                    2 => {
                        current_block_185 = 2262666019490590734;
                        match current_block_185 {
                            11382443704074509989 => {
                                (*self_0).on_visible_node = 0 as libc::c_int != 0;
                                continue;
                            }
                            _ => {
                    ...
                    1 => {
                        current_block_185 = 11382443704074509989;
                        match current_block_185 {
                            11382443704074509989 => {
                                (*self_0).on_visible_node = 0 as libc::c_int != 0;
                                continue;
